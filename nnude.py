...
class Nude(object):
    ...
    def __init__(self, path_or_image):
        ...
        if len(bands) == 1:#if it's a huidutu , turn it to RGB image
            new_img = Image.new("RGB", self.image.size)
            new_img.paste(self.image)
            f = self.image.filename
            self.image = new_img
            self.image.filename = f  #replacement
    ...
    def resize(self, maxwidth=1000, maxheight=1000):
        ...
        if maxwidth:
            if self.width > maxwidth:
                ...
                self.width, self.height = self.image.size
                self.tptal_pixels = self.width * self.height
                ret += 1
        if maxheight:
    ...
    def parse(self):
        ...
        for y in range(self.height):
            for x in range(self.width):
                r = pixels[x, y][0] # attention to the method for getting r,g,b
                g = pixels[x, y][1]
                b = pixels[x, y][2]
                isSkin = True if self._classify_skin(r, g, b) else False
                _id = x + y * self.width +1
                self.skin_map.append(self.Skin(_id, isSkin, None, x, y))
                # manage pixels which is skin-colored
                if not isSkin:
                    continue     # _id starts from 1, so _id's suoyin is _id-1
                check_indexes = [_id - 2,             #left
                                 _id - self.width - 2,#left-ue
                                 _id - self.width - 1,#ue
                                 _id - self.width]    #right-ue
                region = -1
                for index in check_indexes:
                    try:
                        ...
                        break
                    if self.skin_map[index].skin:# if tonarino pixel is skin

                        if (self.skin_map[index].region != None and    # tonarino self.skin_map[index].region is valid
                                region != None and region != -1 and    # current pixel's region is valid
                                self.skin_map[index].region != region and # the two above aren't the same
                                self.last_from != region and    # yet to add the same merging-task
                                self.last_to != self.skin_map[index].region)  :
                            self._add_merge(region, self.skin_map[index].region) # merge the two region (below define)
                        region = self.skin_map[index].region


                if region == -1:# all tonarino pixel aren't skin
                    #change shuxing to new regionnumber
                    _skin = self.skin_map[_id - 1]._replace(region=len(self.detected_regions))
                    self.skin_map[_id - 1] = _skin
                    self.detected_regions.append([self.skin_map[_id - 1]])
                elif region != None: # region is validvalue's tonarino skin-colored pixel
                    # change this region's number to be same to tonarino
    ...
    def _add_merge(self, _from, _to):
        ...
        for index, region in enumerate(self.merge_regions):
            # to judge whether the two already in merge_regions
    ...
    def _analyse_regions(self):
        ...
        if len(self.regions[0]) / total_skin * 100 < 45:
            self.message = "The biggest region contains less than 45 ({:.2f})".format(len(self.regions[0]) / total_skin * 100)
            self.result = False
            return self.result
...
if __name__ == "__main__":
    ...
    parser.add_argument('files', metavar='image', nargs='+', help='Images you want to test')
